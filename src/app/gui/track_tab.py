...
from PySide6.QtWidgets import QWidget, QBoxLayout, QCheckBox, QSpinBox, \
    QComboBox, QLabel, \
...
class MelodyTrackVersion(QWidget):
    ...
    def __init__(self, mf: MainFrame, parent, track_version: TrackVersion,
        ...
        self.piano_roll = PianoRoll(mf=mf, parent=self,
                                    track_version=track_version, synth=synth,
        ...
        self.config_dlg_btn.setDefaultAction(
            self.mf.menu.actions[EDIT_TRACK_VERSION])
    ...
    def populate_preset_combo(self):
        ...
        if (self._preset.itemData(0) and
                self.sf_name == self._preset.itemData(0).sf_name):
            return
        else:
    ...
    def preset(self) -> Preset:
        ...
        return Preset(sf_name=sf_name, bank=self.track_version.bank,
...
class TrackTab(QWidget):
    ...
    def __init__(self, mf, parent, track_version: TrackVersion, synth: FS,
        ...
        self.track_item = MelodyTrackVersion(mf=mf, parent=self,
                                             track_version=track_version,
                                             synth=synth,
                                             composition=composition,
                                             track=self.track)
        self.tab_box.addTab(self.track_item, QIcon(":/icons/piano.png"),
                            "Piano roll")
        self.track_version_table = TrackVersionMidiEvents(mf=mf, parent=self,
                                                          track_version=track_version,
                                                          synth=synth,
                                                          composition=composition,
                                                          track=self.track)
...
class TrackVersionTab(QWidget):
    ...
    def __init__(self, mf: MainFrame, list_item: TrackListItem, track: Track,
        ...
        self.tab_box.tabBar().customContextMenuRequested.connect(
            self.open_menu)
    ...
    def _new_track_version(self, track_version: TrackVersion):
        self.map[track_version.version_name] = TrackTab(mf=self.mf,
                                                        parent=self,
                                                        track_version=track_version,
                                                        synth=self.synth,
                                                        composition=self.composition,
                                                        track=self.track)
        self.tab_box.addTab(self.map[track_version.version_name],
    ...
    def _delete_track_version(self, track_version: TrackVersion):
        self.track.get_version(version_name=track_version.version_name,
    ...
    def on_double_click(self, index):
        self.mf.menu.actions[EDIT_TRACK_VERSION].trigger()

    @property
    def versions(self) -> List[str]:
        return [track_version.version_name for track_version in
                self.track.versions]
    ...
    def open_menu(self, position):
        menu = QMenu()
        menu.addAction(self.mf.menu.actions[NEW_TRACK_VERSION])
        menu.addAction(self.mf.menu.actions[EDIT_TRACK_VERSION])
        menu.setDefaultAction(self.mf.menu.actions[EDIT_TRACK_VERSION])
    ...
    def current_track_version(self, track_version: TrackVersion) -> None:
        if self.tab_box.currentWidget() != self.map[
            track_version.version_name]:
