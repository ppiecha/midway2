...
from PySide6.QtWidgets import QWidget, QTabWidget, QSplitter, QStackedWidget, \
    QBoxLayout
...
class CompositionTab(QWidget):
    ...
    def new_composition(self, composition: Composition):
        ...
        self.map[composition.name] = TrackList(mf=self.mf,
    ...
    def delete_composition(self, composition: Composition):
        ...
        for track_name in list(track_list.map.keys()):
            track_list.delete_track(composition=composition,
                                    track=track_list[track_name].track)
    ...
    def current_track_list(self) -> TrackList:
        ...
        if not track_list:
            raise ValueError(
                f'Cannot determine track list in current composition')
        return track_list
            ...
            for track in self[composition]:
                for track_version in self[composition][track].version_tab:
                    self[composition][track].version_tab[
                        track_version].track_item.init_fonts()
    ...
    def set_keyboard_position(self):
        for composition in self:
            for track in self[composition]:
                for track_version in self[composition][track].version_tab:
                    self[composition][track].version_tab[
                        track_version].track_item.set_keyboard_position()
...
class SequencerBox(QWidget):
    def __init__(self, mf, parent, composition: Composition):
        ...
        self.custom_loop_box = CustomLoopBox(parent=self.splitter, mf=mf,
                                             composition=composition)
        self.splitter.addWidget(self.custom_loop_box)
        self.composition_loop_box = CompositionLoopBox(parent=self.splitter,
                                                       mf=mf,
                                                       composition=composition)
    ...
    def register_listeners(self):
        if not pub.subscribe(self.reload_tracks, REFRESH_LOOPS):
            raise Exception(f"Cannot register listener {REFRESH_LOOPS}")
...
class CustomLoopBox(QWidget):
    def __init__(self, mf, parent, composition: Composition):
        ...
        self.custom_loop_grid = CustomLoopGrid(parent=self, mf=mf,
                                               composition=composition)
...
class CompositionLoopBox(QWidget):
    def __init__(self, mf, parent, composition: Composition):
        super().__init__(parent=parent)
        self.main_box = Box(direction=QBoxLayout.TopToBottom)
        self.composition_loop_grid = CompositionLoopGrid(parent=self, mf=mf,
                                                         composition=composition)
