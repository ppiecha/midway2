            ...
            for (desc, tun) in six.iteritems(_known[x][1]):
                if desc.find(searchd) == 0:
                    if nr_of_strings is None and nr_of_courses is None:
                        ...
                        if tun.count_courses() == nr_of_courses and tun.count_strings() == nr_of_strings:
                            return tun
    ...
    for x in keys:
        if instrument is None or not inkeys and x.find(search) == 0 or inkeys and search == x:
            if nr_of_strings is None and nr_of_courses is None:
                ...
                result += [y for y in six.itervalues(_known[x][1]) if y.count_strings() == nr_of_strings]
            elif nr_of_strings is None and nr_of_courses is not None:
                result += [y for y in six.itervalues(_known[x][1]) if y.count_courses() == nr_of_courses]
            else:
                result += [
                    y
...
add_tuning(
    "Baglamas (Greek)",
    "Modal D standard tuning",
    [["D-4", "D-5"], ["A-4", "A-4"], ["D-5", "D-5"]],
)
add_tuning(
    "Bajo quinto",
    "Standard tuning.",
    [["A-2", "A-1"], ["D-3", "D-2"], ["G-2", "G-2"], ["C-3", "C-3"], ["F-3", "F-3"]],
)
add_tuning(
    "Bajo Sexto",
    "Standard tuning",
...
add_tuning(
    "Bandola Oriental",
    "Standard tuning.",
    [["G-3", "G-3"], ["D-4", "D-4"], ["A-4", "A-4"], ["E-5", "E-5"]],
)
add_tuning(
    "Banjo (bass)",
...
add_tuning(
    "Banjo (cello)",
    "Standard tuning. Same as cello and mandocello",
    ["C-2", "G-2", "D-3", "A-3"],
)
add_tuning("Banjo (tenor)", "Standard tenor jazz tuning", ["C-3", "G-3", "D-4", "A-4"])
add_tuning("Banjo (tenor)", "Irish tenor tuning", ["G-2", "D-3", "A-3", "E-4"])
add_tuning("Banjo (5-string)", "Open G tuning", ["G-4", "D-3", "G-3", "B-3", "D-4"])
add_tuning(
    "Baritone guitar",
...
add_tuning(
    ...
    [["G-4", "G-4"], ["C-4", "C-4"], ["E-5", "E-4"], ["A-4", "A-4"], ["E-5", "E-5"]],
)
add_tuning(
    "Charangon",
    "F6 tuning",
    [["C-4", "C-4"], ["F-4", "F-4"], ["A-5", "A-4"], ["D-5", "D-5"], ["A-5", "A-5"]],
)
add_tuning(
    "Charangon",
    "G6 tuning",
    [["D-4", "D-4"], ["G-4", "G-4"], ["B-5", "B-4"], ["E-5", "E-5"], ["B-5", "B-5"]],
)
add_tuning(
    "Cuatro",
    "Standard tuning",
    [["B-3", "B-2"], ["E-4", "E-3"], ["A-3", "A-3"], ["D-4", "D-4"], ["G-4", "G-4"]],
)
...
add_tuning(
    "Irish bouzouki",
    "Irish tuning (unison pairs)",
    [["G-2", "G-2"], ["D-3", "D-3"], ["A-3", "A-3"], ["D-4", "D-4"]],
)
add_tuning(
    "Irish bouzouki",
    '"Mandolin" tuning (octaves)',
    [["G-3", "G-2"], ["D-4", "D-3"], ["A-3", "A-3"], ["E-4", "E-4"]],
)
add_tuning(
    "Irish bouzouki",
    '"Mandolin" tuning (unison pairs)',
    [["G-2", "G-2"], ["D-3", "D-3"], ["A-3", "A-3"], ["E-4", "E-4"]],
)
add_tuning(
    "Irish bouzouki",
    "Modal D tuning (octaves)",
    [["A-3", "A-2"], ["D-4", "D-3"], ["A-3", "A-3"], ["D-4", "D-4"]],
)
add_tuning(
    "Irish bouzouki",
    "Modal D tuning (unison pairs)",
    [["A-2", "A-2"], ["D-3", "D-3"], ["A-3", "A-3"], ["D-4", "D-4"]],
)
...
add_tuning(
    "Mandocello",
    "Standard tuning. Pitched an octave below the mandola.",
...
add_tuning(
    "Mandolin (piccolo)",
    "Standard tuning",
    [["C-4", "C-4"], ["G-4", "G-4"], ["D-5", "D-5"], ["A-5", "A-5"]],
)
add_tuning(
    "Mandolin (Octave)",
...
add_tuning(
    "Viola",
    "Standard tuning. Pitched a 5th below the violin.",
    ["C-3", "G-3", "D-4", "A-4"],
)
