...
from src.app.model.loop import TrackLoopItem, Loop, LoopType, CompositionLoops
...
def test_play_custom_loop(track_c_major, synth):
    composition = Composition.from_tracks(name="test_scale", tracks=[track_c_major])
    synth.play_loop(
        loops=composition.loops[LoopType.custom],
        loop_name=GuiAttr.DEFAULT,
        bpm=120,
        repeat=False,
    )
...
def test_play_loop_composition(track_c_major, synth):
    composition = Composition.from_tracks(name="test_scale", tracks=[track_c_major])
    ...
    synth.play_loop(
        loops=composition.loops[LoopType.composition],
        loop_name=GuiAttr.FIRST_COMPOSITION_LOOP,
        bpm=120,
        repeat=False,
    )
    while synth.is_playing():
...
def test_play_compiled_loop(track_c_major, synth):
    ...
    synth.play_custom_loop(composition=c, bpm=120)
    while synth.is_playing():
        sleep(0.1)
