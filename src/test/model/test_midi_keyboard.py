...
def test_midi_keyboard_constructor(capsys):
    pad = MidiKeyboard(channel=0)
    key = pad.get_key_by_pitch(pitch=Midi.MAX_B9)
    assert [key.key_top, key.key_bottom] == [0, 14]
    key = pad.get_key_by_pitch(pitch=Midi.MAX_B9 - 1)
    assert [key.key_top, key.key_bottom] == [10, 18]
    key = pad.get_key_by_pitch(pitch=Midi.MAX_B9 - 2)
    assert [key.key_top, key.key_bottom] == [14, 28]
    key = pad.get_key_by_pitch(pitch=Midi.MIN_C1)
