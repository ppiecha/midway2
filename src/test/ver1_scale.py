...
if __name__ == "__main__":
    ...
    sequencer.program_change(time=1000, channel=0, sfid=sfid, bank=0, preset=5, dest=synthSeqID)
    sequencer.program_change(time=2000, channel=0, sfid=sfid, bank=0, preset=10, dest=synthSeqID)
    sequencer.program_change(time=3000, channel=0, sfid=sfid, bank=0, preset=15, dest=synthSeqID)
    for i in range(100):
        sequencer.note(time=int(now + 500 * i),
                       channel=0, key=60 + i,
                       duration=250,
                       velocity=127,
                       dest=synthSeqID)
